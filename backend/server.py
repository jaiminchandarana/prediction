...
def login():
    try:
        ...
        identifier = data.get('identifier')  # Email or ID
...
def register():
    try:
        ...
        required_fields = ['name', 'email', 'password', 'phone', 'address', 'role']
        ...
        # Only allow patient and admin registration through this endpoint
        ...
        try:
            if role == 'patient':
                ...
                hospital = data.get('hospital', 'default hospital').lower()
                
                cur.execute("""
                    INSERT INTO admin (admin_id, first_name, last_name, hospital, phone, email, address, password)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                """, (admin_id, first_name, last_name, hospital, phone, email, address, password_str))
...
def add_patient():
    try:
        ...
        cur.execute("""
            INSERT INTO patient (patient_id, first_name, last_name, phone, email, address, password)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (patient_id, first_name, last_name, data['phone'], data['email'].lower(), 
              data['address'].lower(), password_str))
        ...
        print(f"Error in add_patient: {str(e)}")
        print(traceback.format_exc())
